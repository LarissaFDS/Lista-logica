...
void translateToLogic(char *input, char *output) {
    ...
    while (token != NULL) {
        ...
        if (strcmp(token, "e") == 0 || strcmp(token, "ou") == 0 || 
            ...
            if (strcmp(token, "e") == 0) {
                ...
                // Adiciona a proposição antecedente
                if (strlen(proposition) > 0) {
                    strcat(antecedent, proposition);
                    proposition[0] = '\0';  // Limpa a proposição
                }
                inAntecedent = 1;  // Começa a coletar a proposição consequente
                expectingProposition = 1;
            ...
            strcat(output, "¬");  // Adiciona a negação diretamente
            expectingProposition = 1;  // Depois da negação, espera-se uma proposição

        } else if (strcmp(token, "então") == 0) {
            // Adiciona o conectivo de implicação
            strcat(output, " → ");
            inAntecedent = 0;  // Indica que estamos agora no consequente
            expectingProposition = 1;

        } else {
            // Agrupar palavras em uma proposição
            if (inAntecedent) {
    ...
    // Adicionar a última proposição ao output, se existir
    if (strlen(proposition) > 0) {
        if (inAntecedent) {
            strcat(antecedent, " ");
            strcat(antecedent, proposition);
    ...
    // Adicionar a última proposição após implicação, se existir
